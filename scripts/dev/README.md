...
| [`storage-public-access.sh`](./storage-public-access.sh) | Manually flip a workload Storage account's `publicNetworkAccess` on (caller IPv4 allowlisted, `defaultAction=Deny`, `bypass=None`) / off for local debugging. Normal local launches never open it; the api auto-open path requires an explicit `LOCAL_DEBUG_AUTO_OPEN_STORAGE=true` opt-in. See `api/services/storage/public_access.py`. |
