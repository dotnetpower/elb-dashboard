...
usage() {
  cat >&2 <<'USAGE'
usage: scripts/dev/local-run.sh <start|stop|restart|status|logs|logs-clean|api|worker|beat|web|redis|terminal-exec|smoke|debug-env|storage-on|storage-off|storage-status|auth-on|auth-off|auth-status|compose-full|compose-local> [-- extra args]
...
case "$service" in
  ...
  debug-env)
    ...
    {
      echo "# Generated by scripts/dev/local-run.sh debug-env — do not edit by hand."
