...
test("Dashboard settings, ACR build, and database manager events stay client-safe", async ({
  ...
  const databasesDialog = uiPage.getByRole("dialog", {
    name: "BLAST Databases",
  });
  await expect(databasesDialog).toBeVisible();
  await uiPage.getByTitle("Refresh database status").click();
  await databasesDialog.getByTitle("Close").click();
  await expect(databasesDialog).toHaveCount(0);
});
