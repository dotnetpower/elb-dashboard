...
Reuses the legacy `services.azure_clients` wrappers via `api_app.services`.
All ARM calls run under the api sidecar's managed identity, so the SPA
does not need to acquire ARM-scoped tokens itself for these routes.
"""
...
from api_app.routes.monitor import _graceful  # reuse degraded-response helper
from api_app.services import get_credential

# Bridge legacy services package
import api_app.services as _bootstrap  # noqa: F401

from services.azure_clients import (  # type: ignore  # noqa: E402
    acr_client,
    compute_client,
    resource_client,
    storage_client,
