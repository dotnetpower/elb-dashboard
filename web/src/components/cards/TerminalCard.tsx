...
export function TerminalCard({ subscriptionId, resourceGroup, vmName }: Props) {
  ...
  const [actionLoading, setActionLoading] = useState<"starting" | "stopping" | null>(
    null,
  );
  ...
  const vmNotFound =
    query.isError && (query.error as Error).message?.toLowerCase().includes("not found");
  const otherError = query.isError && !vmNotFound;
  const isProvisioning =
    vmNotFound && Boolean(sessionStorage.getItem("elb-terminal-instance-id"));
  ...
  const handleCopySsh = () => {
    const host =
      ...
      `${vmName}.${query.data?.region}.cloudapp.azure.com`;
  ...
  return (
    <MonitorCard
      ...
      onRefresh={() => {
        query.refetch();
        if (isRunning) healthQuery.refetch();
      }}
        ...
        <div style={{ display: "flex", gap: 6 }}>
          ...
          {(query.data || isProvisioning) && (
            <Link to="/terminal" className="glass-button" style={{ fontSize: 11 }}>
              {isProvisioning ? (
                <>
                  <Loader2 size={12} className="spin" /> View Progress
                </>
              ) : (
                <>
    ...
    >
      {!enabled && (
        <div className="muted">Provision a Remote Terminal to enable monitoring.</div>
      )}
      {isProvisioning && (
      ...
      {otherError && (
        <div className="muted" style={{ color: "var(--danger)" }}>
          Monitoring error: {formatApiError(query.error, "terminal")}
        </div>
      )}
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            <tbody>
              <tr style={{ borderBottom: "1px solid var(--border-weak)" }}>
                <td style={{ padding: "5px 0", color: "var(--text-faint)", width: 90 }}>
                  Power
                </td>
                <td
                  style={{
              ...
              {query.data.os_disk_gb && (
                <tr style={{ borderBottom: "1px solid var(--border-weak)" }}>
                  <td style={{ padding: "5px 0", color: "var(--text-faint)" }}>
                    <HardDrive size={11} style={{ verticalAlign: "middle" }} /> Disk
                  </td>
              ...
              <tr style={{ borderBottom: "1px solid var(--border-weak)" }}>
                <td style={{ padding: "5px 0", color: "var(--text-faint)" }}>
                  <Globe size={11} style={{ verticalAlign: "middle" }} /> IP
                </td>
                <td style={{ padding: "5px 0" }}>
                  {query.data.public_ip ? (
                    ...
                    <span className="muted">
                      No public IP{isStopped ? " (stopped)" : ""}
                    </span>
                  )}
                  {query.data.fqdn && (
              ...
              <tr>
                <td style={{ padding: "5px 0", color: "var(--text-faint)" }}>
                  <Shield size={11} style={{ verticalAlign: "middle" }} /> Identity
                </td>
                <td style={{ padding: "5px 0" }}>
                  {query.data.has_managed_identity ? (
                    <span style={{ color: "var(--success)", fontSize: 11 }}>
                      ✓ {query.data.identity_type}
                    </span>
                  ) : (
              ...
              >
                <span
                  style={{
                    ...
                    borderRadius: "50%",
                    flexShrink: 0,
                    background: healthQuery.data.az_login_active
                      ? "var(--success)"
                      : "var(--danger)",
                  }}
                />
                {healthQuery.data.az_login_active ? (
                  <span>
                    Active —{" "}
                    <span className="muted">{healthQuery.data.az_login_user}</span>
                  </span>
                ) : (
