      ...
      {svcQuery.isError && !clusterStopped && (
        <div style={{
          ...
          <p style={{ color: "var(--text-muted)", fontSize: 12, margin: "0 0 12px" }}>
            Deploy the <code style={{
              fontFamily: "var(--font-mono)", background: "var(--bg-tertiary)",
              padding: "1px 5px", borderRadius: 3,
            }}>elb-openapi</code> service to your AKS cluster from the ACR card on the Dashboard.
          </p>
          <button className="glass-button" onClick={() => svcQuery.refetch()} disabled={svcQuery.isFetching} style={{ fontSize: 12 }}>
            <RefreshCw size={12} /> Retry Discovery
          </button>
        </div>
      )}
