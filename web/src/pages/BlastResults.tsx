...
const FAILURE_PHASES = new Set(["failed", "error", "submit_failed", "warmup_failed"]);
...
function StepLogSection({
  ...
  const getStepLog = (key: string, state: StepState): string | null => {
    ...
    switch (key) {
      ...
      case "submitting": {
        const so =
          (sd.output as string) || ((output as Record<string, unknown>)?.error as string);
        if (state === "error") return `✗ Submit failed:\n${so || failureText}`;
        if (state === "done" && sd.output)
          return `✓ Submitted successfully.\n\n--- Console Output ---\n${sd.output as string}`;
        return state === "done"
          ? "✓ Job submitted to AKS cluster."
          : "Running elastic-blast submit on Remote Terminal VM...";
      }
      case "running": {
