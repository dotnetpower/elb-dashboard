...
!!! note "Tied top scores and near-miss variants"
    ...
    A sharded merge returns at most `max_target_seqs` subjects per query. If that entire window is filled by one `(evalue, bitscore)` tie and the same top class also extends beyond the cutoff, the merge preserves a proportional share of the distinct lower-scoring subjects found in the merged shard candidate pool. For `N = max_target_seqs`, `T` unique top-class subjects, and `L` unique lower-scoring subjects, the automatic reservation is `ceil(N * L / (T + L))`, bounded so at least one top hit remains. This prevents a cross-shard multiplication of perfect ties from hiding every likely variant while preserving the requested subject count. For tabular output, all HSP rows belonging to each selected subject are retained, so the file can contain more than `max_target_seqs` rows. The results notice identifies jobs where reservation occurred and reports the reserved and available candidate counts.

    The merge can only select candidates emitted by the individual shards. Because BLAST+ also applies `max_target_seqs` inside each shard, a candidate already discarded there cannot be recovered; increase `max_target_seqs` when the notice reports a large tied class and broader tail coverage is required. Operators can set `ELB_DIVERSITY_AWARE_CUTOFF=0` in a custom finalizer environment to restore strict score-only top-N selection, `auto` for the default proportional policy, or a positive integer to reserve that many final slots. A strict tie-order oracle always takes precedence and disables diversity reservation. Proportional preservation improves variant visibility but does not claim exact NCBI/full-DB top-N membership when BLAST's internal tied-hit selection order is unavailable.
