...
| Full-DB-exact shard | `precise` | Partitioned search using verified full-DB search-space correction plus BLAST's native e-value/raw-score/database-OID comparator. A ready same-generation DB-order oracle is mandatory; the submit fails closed when it is unavailable. | `outfmt` must be XML 5 or merge-compatible tabular 6/7; tabular runs automatically include raw `score`; the database must carry verified search-space evidence and a complete current order oracle. | [api/services/sharding_precision.py](../../api/services/sharding_precision.py), [docs/research/blast-searchsp-discovery.md](../research/blast-searchsp-discovery.md) |
