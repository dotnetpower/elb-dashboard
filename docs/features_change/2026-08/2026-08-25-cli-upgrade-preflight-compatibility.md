...
- Deployment discovery reads the selected azd environment through the supported `azd env get-value AZURE_ENV_NAME` command before persisting resolved metadata.

## API and infrastructure summary

- No API, image, Bicep, role assignment, or deployed resource contract changed.
- The change is limited to `scripts/dev/cli-upgrade.sh`, `scripts/dev/_caller-precheck.sh`, `scripts/dev/az-context.sh`, and their local regression tests.
...
- `uv run pytest -q api/tests/test_az_context_acr_guard.py -m subprocess` - `4 passed`.
