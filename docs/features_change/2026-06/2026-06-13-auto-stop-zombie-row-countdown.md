...
- **Cluster auto-stops again.** A jobstate row in an active status whose most
  ...
  (default 4 h) is treated as a crashed/`worker_lost` zombie and dropped from
...
- New optional env var `AKS_AUTOSTOP_ACTIVE_ROW_STALE_SECONDS` (default `14400`).
  No infra change required; the default is safe (Celery's hard task time limit
  is 3600 s, so a row untouched for 4 h is provably dead).

## Code changes

- `api/services/auto_stop_evaluator.py` — add `_ACTIVE_ROW_STALE_SECONDS`;
  `_scan_cluster_jobs` now ages out stale active rows (rows with no parseable
  timestamp fail safe and stay counted); `evaluate_cluster` passes `now`.
...
- `uv run pytest -q api/tests` → 3434 passed, 3 skipped.
...
- Live evidence: `/api/aks/autostop/status` returned constant
  ...
  `app=blast` jobs were `Complete` and the only stuck row was the 10 h-old
  `warmup` jobstate row.
