...
def test_local_launchers_do_not_implicitly_enable_auto_open() -> None:
    local_run = (_REPO_ROOT / "scripts/dev/local-run.sh").read_text(encoding="utf-8")
    assert (
        "export LOCAL_DEBUG_AUTO_OPEN_STORAGE=${LOCAL_DEBUG_AUTO_OPEN_STORAGE:-false}"
        in local_run
    )
    assert "LOCAL_DEBUG_AUTO_OPEN_STORAGE=${LOCAL_DEBUG_AUTO_OPEN_STORAGE:-true}" not in local_run

    launch = json.loads((_REPO_ROOT / ".vscode/launch.json").read_text(encoding="utf-8"))
    api_launch = next(
        config
        for config in launch["configurations"]
        if config["name"] == "API: FastAPI (uvicorn)"
    )
    assert "LOCAL_DEBUG_AUTO_OPEN_STORAGE" not in api_launch["env"]
...
def test_ensure_already_open(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv(spa.ENV_OPT_IN, "true")
    sc = MagicMock()
    acct = _make_account("Enabled", ip_rules=["9.9.9.9"])
...
def test_ensure_opens_when_disabled(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert update_params.network_rule_set.bypass == "None"
    assert [rule.ip_address_or_range for rule in update_params.network_rule_set.ip_rules] == [
        "9.9.9.9"
    ]


def test_ensure_refuses_same_region_azure_host_before_update(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv(spa.ENV_OPT_IN, "true")
    sc = MagicMock()
    account = _make_account("Disabled")
    account.primary_location = "Korea Central"
    sc.storage_accounts.get_properties.return_value = account
    with (
        patch("api.services.azure_clients.storage_client", return_value=sc),
        patch.object(spa, "_detect_caller_ip", return_value="52.231.2.38"),
        patch.object(spa, "_detect_azure_host_region", return_value="KoreaCentral"),
    ):
        result = spa.ensure_local_storage_access(MagicMock(), "sub", "rg", "elbstg01")

    assert result["action"] == "failed"
    assert result["error"] == "same_region_azure_ip_rules_unsupported"
    assert "Storage was not changed" in result["message"]
    sc.storage_accounts.update.assert_not_called()


def test_ensure_replaces_unsafe_rules_with_caller_only(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv(spa.ENV_OPT_IN, "true")
    sc = MagicMock()
...
def test_ensure_fails_closed_when_caller_ip_unknown(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv(spa.ENV_OPT_IN, "true")
    sc = MagicMock()
    ...
    with (
        ...
        result = spa.ensure_local_storage_access(MagicMock(), "sub", "rg", "elbstg01")
    assert result == {"action": "failed", "error": "could not detect caller public IP"}
    sc.storage_accounts.get_properties.assert_not_called()
    sc.storage_accounts.update.assert_not_called()
...
def test_shell_toggle_never_uses_allow_or_azure_services_bypass() -> None:
    script = (_REPO_ROOT / "scripts/dev/storage-public-access.sh").read_text(
        encoding="utf-8"
    )
    executable = "\n".join(
        line for line in script.splitlines() if not line.lstrip().startswith("#")
    )
    assert "--default-action Allow" not in executable
    assert "--bypass AzureServices" not in executable
    assert "--default-action Deny --bypass None" in executable
    assert "--ip-address \"$IP\"" in executable
    assert "IP rules do not apply to same-region Azure clients" in executable
