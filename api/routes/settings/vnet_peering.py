...
def _record_audit_event(
    ...
    The bool return is what lets ``_audit_session`` distinguish "backend
...
def _audit_session(
    *,
    ...
    """
    audit_job_id = _record_audit_started(
        op=op,
        caller=caller,
    ...
    def _set_terminal(event: str, payload: dict[str, Any]) -> bool:
        ok = _record_audit_event(audit_job_id, event, payload)
        if ok:
            state["recorded"] = True
        return ok
