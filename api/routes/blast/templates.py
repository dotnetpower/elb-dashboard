...
Risky contracts: Every route enforces ``require_caller``. ``fields`` contains only
known reusable options (never query or execution identity data); validation maps to
400 and Storage failures map to 503 instead of masquerading as missing data.
...
def list_templates_route(
    ...
    from api.services.blast.submit_templates import TemplateStorageError, list_templates

    try:
        templates = list_templates(caller.object_id)
    except TemplateStorageError as exc:
        raise HTTPException(503, "template storage is unavailable") from exc
...
def create_template_route(
    ...
    from api.services.blast.submit_templates import (
...
def delete_template_route(
    ...
    from api.services.blast.submit_templates import TemplateStorageError, delete_template
