...
# ---------------------------------------------------------------------------
# Public-HTTPS routes (enable / disable / status / validator rules) live in a
# sibling module to keep this file focused on deploy / token / spec. They are
# merged onto ``router`` here so ``aks/__init__.py`` keeps including a single
# ``openapi.router``. ``_validate_operator_email`` is re-exported because
...
router.include_router(_public_https_router)
