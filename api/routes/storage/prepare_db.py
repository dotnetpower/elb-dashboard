...
lives in `api/services/storage/prepare_db_{locks,metadata,copy_poller}.py`. The AKS-fanout
dispatch decision + orchestration lives in `api/services/storage/prepare_db_aks_dispatch.py`
(`try_dispatch_aks_mode`); this route only translates its `AksDispatchError` into HTTP.
Key entry points: `prepare_db`, `prepare_db_cancel`, `prepare_db_delete`
...
from api.services.storage.prepare_db_aks_dispatch import (
    AksDispatchError,
    try_dispatch_aks_mode,
)
...
def prepare_db(
    ...
    if raw_mode in {"aks", "auto"}:
        ...
        except AksDispatchError as exc:
            # Translate the service-tier domain error into the HTTP contract.
            # ``aks`` mode with unavailable AKS surfaces as 409 here; the
            # service never silently falls back on explicit ``mode=aks``.
            raise HTTPException(exc.status_code, exc.detail) from exc
        if outcome is not None:
            return outcome
        # auto + AKS not available → ``try_dispatch_aks_mode`` returns None and
        # we fall through to the existing server-side path below.
    ...
    def _do_copies() -> None:
        ...
        try:
            ...
            # Prune ghost volumes left from a previous (larger) NCBI generation
            # and regenerate the shard alias layout for the TRUE volume set —
            # atomically, right after the download. Prevents the 3-way
            # generation mismatch (metadata / volume files / shard layout
            ...
            try:
                ...
                LOGGER.warning(
                    "prepare-db consistency reconcile failed for %s: %s",
                    db_name,
                    sanitise(str(exc))[:200],
                )
