...
Responsibility: Decide whether a submit uses the deployment-wide queue and
    enqueue an accepted request with an immutable execution-target snapshot.
    NO direct OpenAPI submit here (the route owns the fallback), NO drain logic
    (that is the consumer task).
...
Risky contracts: ``should_enqueue_submit`` returns True ONLY when the gate env
    ...
    half-configured deployment never silently drops submits into a void. A
    caller-selected cluster scope stays on the direct path because the queue has
    one deployment-wide target.
    ``enqueue_submit_request`` raises on a real publish failure so the caller can
    fall back to the direct path. Its queue body remains wire-compatible with
    existing Service Bus consumers.
...
def enqueue_submit_request(payload: dict[str, Any], correlation_id: str) -> str:
    ...
    message_id = str(
        service_bus.send_request(
            ...
            subject=build_request_subject(body),
        )
    )
