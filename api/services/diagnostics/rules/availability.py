    ...
    if power.lower() == "stopped":
        return [
            _mk(
                ...
                recommendation=(
                    "Start the cluster from the AKS card before submitting a search."
                ),
                doc_url=_DOC_AKS_NODE,
    ...
    if not pressure.get("reachable", False):
        return [
            _mk(
                ...
                recommendation=(
                    "Re-run once the cluster is reachable; check the kubeconfig / "
                    "network path."
                ),
                doc_url=_DOC_AKS_NODE,
    ...
    if hot:
        ...
        return [
            _mk(
                ...
                recommendation=(
                    "Scale the pool out or enable the cluster autoscaler so pods "
                    "can schedule."
                ),
    ...
    if down:
        findings.append(
            _mk(
                ...
                recommendation=(
                    "Inspect the sidecar logs; a down api/worker/redis blocks "
                    "dashboard actions."
                ),
                doc_url=_DOC_CONTAINER_APP,
        ...
        findings.append(
            _mk(
                ...
                detail="; ".join(parts) + ".",
                recommendation=(
                    "Watch the affected sidecars; sustained pressure may slow the "
                    "dashboard."
                ),
                doc_url=_DOC_CONTAINER_APP,
    ...
    if error_rate >= _API_ERROR_RATE_CRIT:
        findings.append(
            _mk(
                ...
                recommendation=(
                    "Inspect the failing routes in the HTTP inspector; this is an "
                    "active outage."
                ),
                doc_url=_DOC_PERF,
    ...
    if isinstance(p95, (int, float)) and float(p95) >= _API_P95_WARN_MS:
        findings.append(
            _mk(
                ...
                detail=(
                    f"95th-percentile latency exceeds {int(_API_P95_WARN_MS)} ms "
                    "over the window."
                ),
                recommendation=(
                    "Investigate slow routes; check ARM throttling and downstream "
                    "timeouts."
                ),
                doc_url=_DOC_PERF,
